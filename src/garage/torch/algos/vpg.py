...
class VPG(BatchPolopt):
    ...
    def __init__(
        ...
        self.optimizer = optimizer(policy.get_module().parameters(),
                                   lr=policy_lr)

        filter = torch.full((1, 1, 1, max_path_length), discount * gae_lambda)
        self.filter = torch.cumprod(filter, dim=-1)
    ...
    def _optimize_policy(self, itr, paths):
        ...
        policy_entropies = torch.stack([
            self._add_padding(
                ...
                self.max_path_length)  # noqa: E501
            for path in paths
        ])
        ...
        if self.maximum_entropy:
            rewards = [
                reward + self.policy_ent_coeff * policy_entropies
                for reward in rewards
            ]

        deltas = (rewards + self.discount * baselines -
                  F.pad(baselines, (1, 0))[:, :-1])
        deltas = F.pad(deltas,
                       (0, self.max_path_length - 1)).unsqueeze(0).unsqueeze(0)

        advantages = F.conv2d(deltas, self.filter, stride=1).squeeze()

        if self.center_adv:
            advantages = F.batch_norm(advantages.reshape((-1, 1)),
        ...
        log_likelihoods = torch.stack([
            self._add_padding(
                self.policy.get_log_likelihood(
                    torch.Tensor(path['observations']),
                    torch.Tensor(path['actions'])), self.max_path_length)
            for path in paths
        ])

        outputs = advantages * log_likelihoods

        if self.entropy_regularzied:
            outputs += self.policy_ent_coeff * policy_entropies

        loss = torch.cat(
            [outputs[idx, :valids[idx]] for idx in range(outputs.shape[0])])
        (-loss).mean().backward()
